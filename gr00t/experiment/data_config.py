...
class Gr1ForG1ArmsOnlyTuningDataConfig(BaseDataConfig):
    ...
    def transform(self) -> ModalityTransform:
        transforms = [
            ...
            VideoToNumpy(apply_to=self.video_keys),
            # state transforms
            PostStateActionRetarget(apply_to=self.state_keys),
            StateActionToTensor(apply_to=self.state_keys),
            StateActionSinCosTransform(apply_to=self.state_keys),
            #StateActionTransform(
            #    apply_to=self.state_keys,
...
class Gr1ForG1TuningDataConfig(BaseDataConfig):
    video_keys = ["video.rs_view"]
...
class G1ArmsOnlyFinetuneDataConfig(BaseDataConfig):
    video_keys = ["video.rs_view"]
    state_keys = [
        "state.left_arm",
        "state.right_arm",
...
class G1FinetuneDataConfig(BaseDataConfig):
    video_keys = ["video.rs_view"]
    state_keys = [
        "state.left_arm",
        "state.right_arm",
...
class Dex31G1ArmsOnlyDataConfig(BaseDataConfig):
    video_keys = ["video.rs_view"]
    state_keys = ["state.left_arm", "state.right_arm", "state.left_hand", "state.right_hand"]
    action_keys = ["action.left_arm", "action.right_arm", "action.left_hand", "action.right_hand"]
    language_keys = ["annotation.human.task_description"]
...
class G1LeftArmDataConfig(BaseDataConfig):
    video_keys = ["video.rs_view"]
...
class UnitreeG1LocoUpperDataConfig(UnitreeG1DataConfig):
    video_keys = ["video.rs_view"]
    state_keys = [
        "state.left_arm",
        "state.right_arm",
...
DATA_CONFIG_MAP = {
    ...
    "fourier_gr1_full_body": FourierGr1FullBodyDataConfig(),
    ...
    "unitree_g1_loco_upper": UnitreeG1LocoUpperDataConfig(),
    ...
    "g1_to_gr1_armsonly": Gr1ForG1ArmsOnlyTuningDataConfig(),    # Fine-tune GR1 Policy with G1 data
    "g1_finetune": G1FinetuneDataConfig(),      # Fine-tune G1 policy with G1 data
    "g1_finetune_armsonly": G1ArmsOnlyFinetuneDataConfig(),      # Fine-tune G1 policy with G1 data
